...
appStickerPipeStore.factory('Api', function(Http, EnvConfig, Config, DataCache, $q, LocalStorage) {
    ...
    return {
        ...
        getPacks: function() {
			var storedPacks = LocalStorage.get('packs'),
				storedPacksExpire = LocalStorage.get('packsExpires');

			if (!storedPacks || storedPacksExpire < +(new Date())) {

				return Http.get(getUrl('shop')).then(function(response) {

					if (!storedPacks || storedPacksExpire < response.meta.shop_last_modified) {
						var packs = response.data || response;

						LocalStorage.set('packs', packs);
						LocalStorage.set('packsExpires', (new Date()).setDate((new Date()).getDate() + 1));
...
appStickerPipeStore.factory('LocalStorage', function() {
